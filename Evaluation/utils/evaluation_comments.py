...
class PlotSetup:
    ...
    ratio_ylabel: str = '$\frac{DT 2.1}{DT 2.5}$'
    ...
    def add_text(self, ax, n_entries, pt_min, pt_max, eta_min, eta_max, dm_bin, period):
        ...
        ax.text(0.03, 0.75 - n_entries*0.07, "Jets from "r'$W+jets$', fontsize=14, transform=ax.transAxes)
        ...
        ax.text(0.65, header_y, '59.7 $fb^{-1}$ (13 TeV)', fontsize=14, transform=ax.transAxes,
                fontfamily='sans-serif')
...
class Discriminator:
    ...
    def count_passed_threshold(self, df, threshold):
        # if self.wp_from == 'wp_column':
        #     assert self.wp_column in df.columns
        #     wp = self.wp_name_to_index[wp_name]
        #     flag = 1 << wp
        #     passed = (df[self.wp_column] >= threshold).astype(int)
        #     return np.sum(passed * df.weight.values)
        # elif self.wp_from == 'pred_column':
        #     if len(self.wp_thresholds) > 0:
        #         assert self.pred_column in df.columns
                # wp_thr = self.wp_thresholds[wp_name]
        return np.sum(df[df[self.pred_column] > threshold].weight.values)
            # else:
            #     raise RuntimeError('Working points thresholds are not specified for discriminator "{}"'.format(self.name))
        # else:
        #     raise RuntimeError(f'count_passed() behaviour not defined for: wp_from={self.wp_from}')
        ...
        if self.raw: # construct ROC curve
            ...
            if not (np.isnan(fpr).any() or np.isnan(tpr).any()):
                ...
                roc_cfg = {
                    ...
                    'auc_score': auc_score,
                    'false_positive_rate_up': np.empty(len(fpr)),
                    # 'true_positive_rate_up': np.empty(len(fpr)),
                    'false_positive_rate_down': np.empty(len(fpr)),
                    # 'true_positive_rate_down': np.empty(len(fpr)),
                }
                for wp_i in range(len(fpr)):
                    for kind, pr_name in zip([0], ['false_positive_rate', ]):
                        df_x = df[df['gen_tau'] == kind]
                        n_passed = self.count_passed_threshold(df_x, thresholds[wp_i])
                        n_total = np.sum(df_x.weight.values)
                        eff = float(n_passed) / n_total if n_total > 0 else 0.0
                        # roc_cfg[pr_name][len(fpr) - wp_i - 1] = eff

                        # up/down variations
                        ci_low, ci_upp = proportion_confint(n_passed, n_total, alpha=1-0.68, method='beta')
                        roc_cfg[f'{pr_name}_down'][len(fpr) - wp_i - 1] = ci_upp - eff
                        roc_cfg[f'{pr_name}_up'][len(fpr) - wp_i - 1] = eff - ci_low
